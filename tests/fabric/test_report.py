        ...
        "sections": [
            {
                ...
                "visualContainers": [
                    ...
                    },
                    {
                        "config": {
                            "name": "visual-002",
                            "singleVisualGroup": {"displayName": "Filter Panel"},
                        }
                    },
                    {
                        "config": {
                        ...
                        "title": [
                            {
                                "properties": {
                                    "text": {
                                        "expr": {"Literal": {"Value": "'Sales Table'"}}
                                    }
                                }
                            }
                        ]
...
class TestGetModelMeasureReferences:
    ...
    def test_deduplicates(self, rpt):
        ...
        measures = [
            {
                "references": {
                    "measures": [{"entity": "A", "name": "X", "schema": "model"}]
                }
            },
            {
                "references": {
...
class TestGenerateDaxQueryScript:
    ...
    def test_single_measure(self, rpt):
        measures = [{"entity": "T", "name": "M", "expression": "1+1", "references": {}}]
        script = rpt._generate_dax_query_script(measures)
...
class TestGetLegacyReportPagesAndVisuals:
    ...
    def test_extracts_visuals(self, rpt):
        ...
        df = rpt.get_legacy_report_pages_and_visuals(
            SAMPLE_REPORT_JSON, "ws-1", "rpt-1"
        )
    ...
    def test_visual_container_config_as_string(self, rpt):
        ...
        df = rpt.get_legacy_report_pages_and_visuals(
            json_with_string_configs, "ws-1", "rpt-1"
        )
        assert len(df) == 1
...
class TestGetReportDefinition:
    ...
    @patch("msdev_kit.fabric.report.requests.post")
    @patch("msdev_kit.fabric.report.Operations")
    @patch("msdev_kit.fabric.report.sleep")
    def test_get_report_definition_with_operations_none(
        self, mock_sleep, mock_operations_cls, mock_post, rpt
    ):
        mock_response = MagicMock()
        mock_response.status_code = 202
        mock_response.content = b"{}"
        mock_response.headers = {"x-ms-operation-id": "op-123"}
        mock_post.return_value = mock_response

        mock_ops = MagicMock()
        mock_ops.get_operation_state.return_value = {"operation_state": "Succeeded"}
        mock_ops.get_operation_result.return_value = {
            "content": {"definition": {"format": "PBIR", "parts": []}}
        }
        mock_operations_cls.return_value = mock_ops

        res = rpt.get_report_definition("ws-123", "rpt-456", None)

        assert res["message"] == "Success"
        assert res["content"]["format"] == "PBIR"
        mock_operations_cls.assert_called_once_with("fake-token")
        mock_post.assert_called_once()
        mock_ops.get_operation_state.assert_called_once_with("op-123")
