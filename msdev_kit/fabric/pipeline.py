...
class Pipeline:
    ...
    def _resolve_pipeline(self, workspace_id: str, pipeline_id_or_name: str) -> tuple:
        ...
        if result.get("message") == "Success":
            ...
            return content.get("id", pipeline_id_or_name), content.get(
                "displayName", ""
            )
    ...
    def _resolve_dataflow_id(
        self, workspace_id: str, dataflow_id_or_name: str
    ) -> tuple:
        """
    ...
    def list_pipelines(self, workspace_id: str) -> Dict:
        ...
        api_url = (
            f"{self.fabric_api_base_url}/v1/workspaces/{workspace_id}/dataPipelines"
        )
        pipelines = []
    ...
    def update_pipeline_definition(
        self, workspace_id: str, pipeline_id: str, definition: Dict
    ) -> Dict:
        """
        ...
        if response.status_code in (200, 202):
            ...
            return {
                "message": {"error": error_message, "status_code": response.status_code}
            }

    def replace_dataflow_id_in_pipeline(
        self,
        workspace_id: str,
        pipeline_id: str,
        old_dataflow_id: str,
        ...
        if not pipeline_content:
            return {
                "message": "No pipeline-content.json found in definition.",
                "content": "",
            }
        ...
        print(
            f"Replacing dataflow ID in {updated_count} activity(ies): {', '.join(updated_names)}"
        )
        update_result = self.update_pipeline_definition(
            workspace_id, pipeline_id, definition
        )
    ...
    def get_pipeline_definition(self, workspace_id: str, pipeline_id: str) -> Dict:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error getting pipeline definition: {response.status_code} - {error_message}"
            )
            return {
                "message": {"error": error_message, "status_code": response.status_code}
            }
    ...
    def get_pipeline_activities(
        ...
        if pipeline_id_or_name == "":
            return {
                "message": "Missing pipeline id or name, please check.",
                "content": "",
            }

        # Resolve pipeline ID and name
        pipeline_id, pipeline_name = self._resolve_pipeline(
            workspace_id, pipeline_id_or_name
        )
        if not pipeline_id:
            return {
                "message": f"Pipeline not found: {pipeline_id_or_name}",
                "content": "",
            }
