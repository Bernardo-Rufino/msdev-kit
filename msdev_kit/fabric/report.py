...
class Report:
    ...
    def get_report_metadata(self, workspace_id: str = "", report_id: str = "") -> Dict:
        """
    ...
    def get_report_name(self, workspace_id: str, report_id: str) -> str:
        ...
        """
        report_name = (
            self.get_report_metadata(workspace_id, report_id).get("content").get("name")
        )
    ...
    def list_report_pages(self, workspace_id: str = "", report_id: str = "") -> Dict:
        """
    ...
    def get_legacy_report_pages_and_visuals(
        self, json_data: str, workspace_id: str, report_id: str
    ) -> pd.DataFrame:
        """
        ...
        for i, section in enumerate(sections):
            ...
            for vc in visual_containers:
                ...
                if single_visual_group:
                    ...
                    visual_title = single_visual_group.get(
                    ...
                    if single_visual:
                        ...
                        visual_title = (
                            found_title if found_title != "No Title" else visual_type
                        )
                ...
                if visual_title == visual_type:
                    ...
                    if title_obj and not self._get_nested_value(
                        title_obj, ["Literal", "Value"]
                    ):
    ...
    def get_pbir_report_pages_and_visuals(
        self, parts: List[Dict], workspace_id: str, report_id: str
    ) -> pd.DataFrame:
        """
        ...
        pages_meta = json.loads(
            base64.b64decode(pages_meta_part["payload"]).decode("utf-8")
        )
        ...
        for page_index, page_id in enumerate(page_order, start=1):
            ...
            page_data = json.loads(
                base64.b64decode(page_part["payload"]).decode("utf-8")
            )
    ...
    def get_report_pages_and_visuals(
        self, workspace_id: str, report_id: str, operations: Operations
    ) -> pd.DataFrame:
        """
        ...
        definition_result = self.get_report_definition(
            workspace_id, report_id, operations
        )
        ...
        if fmt == "pbir":
            return self.get_pbir_report_pages_and_visuals(
                parts, workspace_id, report_id
            )
            ...
            if not report_payload:
                print(
                    "get_report_pages_and_visuals: report.json not found in PBIR-Legacy parts."
                )
                return pd.DataFrame()
            ...
            return self.get_legacy_report_pages_and_visuals(
                json_data, workspace_id, report_id
            )

        else:
            print(
                f'get_report_pages_and_visuals: unsupported format "{metadata_format}".'
            )
            return pd.DataFrame()

    def get_report_definition(
        self, workspace_id: str = "", report_id: str = "", operations: Operations = None
    ) -> Dict:
        """
        ...
        if operations is None:
            ...
            operations = Operations(self.token)
        ...
        report_content = operations.get_operation_result(operation_id).get(
            "content", ""
        )
    ...
    def get_legacy_report_json(
        ...
        def _decode_base64_json_to_file(
            encoded_str: str, workspace_id: str, report_id: str
        ) -> None:
            """
        ...
        # Continue...
            ...
            if status == 202:
                ...
                while True:
                    active_operation_state = operations.get_operation_state(
                        operation_id
                    )
                    print("Operation state:", active_operation_state)
                    if active_operation_state:
                        operation_state = active_operation_state.get(
                            "operation_state", ""
                        )
                        if operation_state in ("Succeeded", "Failed"):
                            sleep(1)
                ...
                if report_format.lower() == "pbir-legacy":
                    ...
                    report_json = _decode_base64_json_to_file(
                        report_json_byte_string, workspace_id, report_id
                    )
                ...
                # Report not on Legacy format.
                    ...
                    return {
                        "message": {
                            "error": "Report format invalid, only PBIR-Legacy is supported.",
    ...
    def export_report(
        ...
        # If workspace ID and report ID were informed...
            ...
            if status == 200:
                ...
                return {
                    "message": {"error": f"Error with status code {status}"},
                    "content": "",
                }
    ...
    def get_report_measures(
        ...
        measures_export = [
            {k: v for k, v in m.items() if k != "references"} for m in measures
        ]
    ...
    def rebind_report(
        ...
        if not new_dataset_workspace_id:
            return {
                "message": "Missing new dataset workspace id, please check.",
                "content": "",
            }
        ...
        read_users = [
            u for u in report_users if u.get("reportUserAccessRight") == "Read"
        ]

        # Step 2 — rebind the report
        request_url = (
            f"{self.main_url}/groups/{workspace_id}/reports/{report_id}/Rebind"
        )
        r = requests.post(
            url=request_url, headers=self.headers, json={"datasetId": new_dataset_id}
        )
        ...
        with ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(_grant_access, user): user for user in read_users
            }
            for future in as_completed(futures):
                ...
                if result.get("message") == "Success":
                    ...
                    users_failed.append(
                        {"identifier": identifier, "error": result.get("message")}
                    )
